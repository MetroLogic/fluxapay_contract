...
describe("FLUXAPAY_CONTRACT_ERROR_MAP / toFluxapayError", () => {
    test("every code in the map produces a FluxapayError with the correct code and message", () => {
        for (const [codeStr, name] of Object.entries(FLUXAPAY_CONTRACT_ERROR_MAP)) {
            ...
            assert.equal(err.contractErrorName, name, `code ${code}: contractErrorName should be "${name}", got "${err.contractErrorName}"`);
            assert.equal(err.message, `${name} (contract error #${code})`, `code ${code}: unexpected error message`);
    ...
    test("a plain Error with no recognizable contract-error shape is rethrown as-is", () => {
        ...
        assert.throws(() => toFluxapayError(original), (thrown) => thrown === original);
    });
});
describe("test_all_documented_codes_present_in_map", () => {
    test("every code documented in the Core table of docs/error-codes.md is a key in the SDK map", () => {
        ...
        const coreSectionMatch = docsSource.match(/## Core: `PaymentProcessor` \/ `RefundManager`[\s\S]*?\n(?=## )/);
        ...
        assert.deepEqual(missingFromMap, [], `Codes documented in docs/error-codes.md but missing from FLUXAPAY_CONTRACT_ERROR_MAP: ${missingFromMap.join(", ")}`);
        assert.deepEqual(missingFromDocs, [], `Codes in FLUXAPAY_CONTRACT_ERROR_MAP but not documented in docs/error-codes.md: ${missingFromDocs.join(", ")}`);
    });
});
