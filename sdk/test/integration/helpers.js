...
export function getTestnetEnv() {
    ...
    if (!contractId) {
        throw new Error("TESTNET_RPC_URL is set but TESTNET_PAYMENT_PROCESSOR_CONTRACT_ID is missing — " +
    ...
    if (!adminSecret || !merchantSecret || !payerSecret) {
        throw new Error("TESTNET_RPC_URL is set but one or more of TESTNET_ADMIN_SECRET_KEY / " +
 ...
 */
export function makeTestnetClient(env) {
    const config = {
        network: "testnet",
        rpcUrl: env.rpcUrl,
 ...
 */
export async function signAndSend(tx, signer) {
